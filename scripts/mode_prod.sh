...
SRV_NAME=rlx
UNIT_FILE=/etc/systemd/system/${SRV_NAME}.service
...
sudo useradd --system --shell /usr/sbin/nologin --home-dir /dev/null rlxsvc 2>/dev/null || true
...
sudo chmod -R u+rwX,g+rX,o-rwx "$ROOT/local_bundle"
...
sudo systemctl enable --now "$SRV_NAME"
systemctl status "$SRV_NAME" --no-pager || true
