...
def local_assets():
    ...
    for path in REQ:
        if not os.path.exists(path): rep["missing"].append(path)
        else: rep["present"].append(path); rep["hashes"][path]=sha256f(path)
    for path in OPT:
        if os.path.exists(path):
            rep["present"].append(path); rep["hashes"][path]=sha256f(path)
            try:
        ...
        for filepath in listf(root):
            ...
            for lib in BANNED_NET:
                if re.search(rf'(?m)^\s*(from\s+{re.escape(lib)}\s+import|import\s+{re.escape(lib)}\b)|require\(["\']{re.escape(lib)}["\']\)', txt):
                    hits.append(lib)
            if hits: f["files_with_net_libs"].append({"file":filepath,"libs":sorted(set(hits))})
            urls = REMOTE_URL.findall(txt)
            if urls: f["files_with_remote_urls"].append({"file":filepath,"urls":sorted(set(urls))[:10]})
    return f
def main():
